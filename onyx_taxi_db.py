from sqlalchemy import Column, Integer, String, create_engine, Boolean, ForeignKey, DateTime, func
...
class Driver(Base):
    ...
    car = Column(String, comment="автомобиль водителя")

    # переопределение выдачи объекта на экран через print
    def __repr__(self):
        return "<Player(name={0}, car={1})>".format(self.name, self.car)
...
class Order(Base):
    ...
    client_id = Column(Integer, ForeignKey('client_id'), nullable=False, comment="Идентификатор клиента")
    driver_id = Column(Integer, ForeignKey('driver_id'), nullable=False, comment="Идентификатор водителя")
